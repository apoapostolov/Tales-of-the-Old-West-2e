...
- **Pressure:** the forbidden power is *useful* — it refuels Willpower/Faith (P2), boosts a roll, or buys off a consequence you couldn't otherwise escape. The pressure is the *need* the power answers (a fight going badly, a resource depleted, a failure that costs everything). Crucially, the need must be *real and recurring* — if legitimate options are always sufficient, the spiral never opens (see Failure modes).
