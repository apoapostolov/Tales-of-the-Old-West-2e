...
> **STATUS: FILLED (Pass-1 extraction + Pass-2 abstraction complete).** This is the conceptual backbone. Every other reference file's "Design intent" section and the playtest checks in `13-balance-and-synergy.md` draw from this file. If a rule cannot be defended against the principles here, it does not belong in a YZE-family game.
...
- `skills/forbidden-lands-design/references/system-design-map.md:310-351` — Proposal Integration Method, **Warning Signs** (rule bloat, silent talent invalidation, false realism, vague enforcement, misleading survivability).
...
Abstract the **design philosophy** that makes YZE *work* — the reason both books, despite different genres, feel like the same engine. The thesis: **the engine ties ambition to exposure.** Every loop turns a resource (time, body, gear, Willpower/Faith, safety) into pressure, and pressure into decision. The deliverable is the **pressure economy** model, the **five core loops** (Expedition / Conflict / Recovery / Willpower / Base) restated genre-neutrally, the **cost-of-safety axiom**, the **lethality-as-feature** stance, the **authenticity lens**, the **general/situational/optional layering principle**, and — most critically — the **warning sign list** that the playtest checks in `13-balance-and-synergy.md` use.

Apply the Abstraction Ladder to: identity, pressure, loops, axioms, lethality, tone, layering, warning signs.
...
| **Willpower / Faith** | Willpower (cap 10, harm-refueled) | Faith (cap 10, 4/scenario, action-refueled) | The capped pool that converts risk into future agency |
...
2. **Decision:** the player (or GM) chooses how to respond — push or accept, fight or flee, press on or make camp, spend Willpower/Faith or hoard it.
...
### 4. The Willpower loop

**Sequence (FL `system-design-map.md:86-92`):** attempt difficult action → push → accept risk from banes → gain Willpower → spend it on talents, spells, or pressure relief.
...
**Layer:** all five loops are **General** — a YZE-family game is incomplete without all five operating, though a given genre may weight one or two as primary (FL weights Expedition + Base; West weights Conflict + Recovery). Which loop dominates is a rule choice — see §12.
...
**Why this is a feature:** Without the threat of real loss, the Pressure Economy has no teeth — players would simply push until they succeed, never retreat, never spend resources on avoidance. Lethality is what makes "do we fight or do we flee" a real question, what makes recovery matter, what makes the Willpower loop dramatic rather than mechanical. **Layer:** General (attrition always on); the death/Broken calibration is a rule choice — see §11, §12. Cross-link: `04-harm-and-consequences.md`.
...
- **General** — the core resolution and economy, always on, every table. The dice grammar (6 = success), pushing, the success ladder, the Willpower/Faith pool, the five loops. A player who knows only the General layer can play the game.
...
**The layering discipline (design rule):** When proposing a new rule, first ask which tier it belongs in. A rule that *could* be Situational or Optional but is written as General is a Warning Sign (rule bloat, §10a). A rule that is General but reads as an exception to another General rule is almost always mis-layered. **Layer:** the layering principle itself is General; the assignment of any given rule to a tier is a design decision that must be stated explicitly, not left implicit.

## 10. Warning signs

> This is the engine's warning sign list. **Directly inherited from FL's `system-design-map.md:331-351`, rephrased into portable RPG vocabulary, with two additions (f, g) the skill adds.** The playtest checks in `13-balance-and-synergy.md` check every proposal against this list. A proposal that trips a warning sign is not auto-rejected — it is flagged for deeper review.
...
**Generic:** The proposal introduces a new resource track, die type, condition, or rule set that duplicates a job an existing rule already does. Before adding, ask: *which existing rule could carry this load if it were extended?* FL's own guidance — "does the rule create new roll logic that should instead reuse the base grammar" `system-design-map.md:155` — is the canonical test. **Layer test:** if the rule belongs in Optional and is written as General, it bloats.
...
### f. Wrong naming layer

**Generic (the skill's own contribution):** A rule written at the wrong naming layer. Engine-facing notes may say "capped agency pool" if needed, but player-facing rules should say **Willpower**, **Faith**, **Nerve**, **Hope**, or another living name. Setting-only terms such as "the Blood Mist," "Quarter Day," or "the Stronghold" should be abstracted unless the new game is using that setting. Flavorful generic terms such as **Race**, **Life Events**, **Willpower**, **Holdings and Territory**, and **Riches in the Land** are allowed because they are portable RPG language. Use `26-naming-the-game.md` for the naming pass.

### g. Sibling-skill duplication (skill addition)

**Generic (the skill's own contribution):** Two design skills (here: `forbidden-lands-design` and `western-rpg-design`) independently reinvent the same engine-level rule, audit method, or warning sign in their own vocabulary, producing drift between sibling games. The YZE-design skill exists *precisely* to prevent this: the engine-level rule should be stated once here, and each sibling skill should cite it rather than re-derive it. A proposal that re-derives an engine rule in setting-specific language, rather than instantiating the shared one, is sibling-skill duplication. (This warning sign is meta — it governs the *skills*, not the *rulebooks* — but it is what keeps the two books recognizably the same engine.)
...
| **Optional-rule surface** | Larger (heroic campaigns, artifact/Pride dice, Surge of Willpower, stronghold) | Leaner (Fast Initiative, Consumables as Resource Dice) | More optional rules vs leaner core | Larger surface for high-fantasy/pulp; leaner for grounded/low-power |
...
To build a new YZE-family game, set these philosophy choices (each has FL and West as two calibrated points):
...
5. **Decide the optional-rule surface** — larger options menu (FL) vs lean core (West). *(Sets how many variant levers the table gets.)*
6. **Wire the resource-at-risk model** — confirm each pressure source (time/body/gear/Willpower/safety/overreach) has a four-beat loop (pressure→decision→consequence→state change). If any source lacks one, it is decorative. *(Checks the Pressure Economy.)*
...
1. Write the thesis sentence — ambition = ___, exposure = ___. If you cannot fill both blanks with physical, credible nouns, the genre is not ready for the engine.
2. Set the lethality and the authenticity strictness *together* — they must agree (a documentary genre wants attrition-and-circumstance; an inventive genre can afford a cleaner breakpoint).
3. Weight the loops to match the genre's stories.
...
5. Decide the optional surface from the table's complexity tolerance.
...
- **Does it avoid all seven Warning Signs** — no rule bloat, no silent invalidation, no false realism, no vague enforcement, no misleading survivability, no wrong naming layer, no sibling-skill duplication? (§10)
