...
When you’re trying to achieve something challenging, trouble is never far away. Before a roll, the GM may say the attempt is `Safe`, `Risky`, or `Desperate`. You then roll your normal dice pool only. There are no separate Trouble dice.

#### Exposure and Trouble

#### An Example Dice Pool

A result of `1` on an ordinary die may matter when a roll is `Risky` or `Desperate`. Trouble is only worked out after the roll, and only if the roll failed or the fiction makes a bad turn matter at once. You can use your Faith to reduce the Trouble you may face (page 40).

> _Brody Anaheim, played by Dave, is galloping his horse away from some men out to do him harm, and he needs to make an ANIMAL HANDLIN’ roll. His ANIMAL HANDLIN’ ability score is 3, and his Cunning is 4, giving him a total of seven dice in his pool. The GM says this is a `Risky` roll, because failure means a hard spill or the bad men gaining ground, so Dave rolls all seven dice and keeps an eye on both successes and any `1`s._

#### Sixes Mean Success

To succeed with your action you must roll at least one 6, called a success. A 6 on any die counts as a success. If you roll more than one success your action has worked out even better than you’d hoped and you can achieve other additional effects.
...
If your reserve of Faith runs out (your Faith Points go down to zero) you have shaken your Faith and must restore it. Naturally, without Faith you cannot push your rolls or reduce Trouble with Faith.

#### Pushing your roll can be troublesome

Relying on your Faith and pushing your roll can be troublesome. `Safe` rolls do not normally generate Trouble from the dice themselves unless the fiction is already ugly or reckless. `Risky` and `Desperate` rolls do.

Work out Trouble like this:

- `Safe`: No Trouble from the roll itself unless the GM judges that noise, haste, bloodshed, dynamite, panic, or some similar choice made immediate fallout likely.
...
- If a pushed roll still fails, mark `1` additional Trouble.

Most of the time, only failed `Risky` or `Desperate` rolls need this counting. Successful rolls usually ignore `1`s unless the fiction says the success came at an ugly cost.
...
> _With Brody Anaheim still chased by those bad men, Dave has to make another ANIMAL HANDLIN’ roll with his pool of 7 dice. The GM again calls it `Risky`. This time the roll fails and shows three `1`s. That means `1` Trouble for the failure, plus `1` more Trouble because every two `1`s on a `Risky` roll add another point, for a total of `2` Trouble. Damn, Brody needs to clear that thicket to escape, so Dave pushes the roll, spending 1 Faith Point. The pushed roll still fails, so the Trouble rises by `1` more, to `3`. Dave spends 1 more Faith Point to buy the Trouble back down to `2`, then rolls on the second column of the physical Trouble table. He gets a `1`, which means Brody suffers 1 point of either Hurts or Shakes. The GM decides that Shakes seems right: Anaheim nearly makes the jump, but the horse crashes through a patch of rattlers and the scare rattles him worse than the fall._

**Trouble Outcome Table - Conflict / Physical**

| D6  | 1                                                                                                                                                                                                  | 2                                                                                                                                                                                                            | 3                                                                                                                                                                                                       | 4+                                                                                                                                                                                                |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Your gun or weapon suffers a random Condition, or an important item breaks.                                                                                                                        | You are bashed and battered. Suffer 1 point of either Hurts or Shakes (GM decides).                                                                                                                          | You catch yourself on your stirrup or other sharp object or bash into something hard. You suffer a 4 dice attack (Damage 1, Crit 2).                                                                    | You’re so shaken and shocked by events that, for the next Shift, all your rolls are treated as one step more exposed: `Safe` becomes `Risky`, `Risky` becomes `Desperate`, and `Desperate` rolls gain `+1` Trouble on a failure. You may recover by taking a Turn to draw breath and spending 2 Faith Points. |
...
| 4   | Your attack is underpowered, or your action is weak. Lose one success from your total.                                                                                                             | You’re badly shaken by the turn of events. Until the end of the scene, all your rolls are treated as one step more exposed: `Safe` becomes `Risky`, `Risky` becomes `Desperate`, and `Desperate` rolls gain `+1` Trouble on a failure. You may recover from this by taking a slow action to draw breath, and spending a Faith Point. | Your attack or action affects a random bystander or unintended target. Either apply the effect of the attack to that target, or they suffer a 6 dice attack (Damage 1, Crit 1).                         | Your attack or action has terribly affected a random nearby bystander. They are immediately Broken on Grit and suffer a critical injury.                                                          |
...
| 4   | They know you’re faking it, and you know they know you’re faking it. Lose one success from your total.                                                                                     | You’re badly shaken by the turn of events. Until the end of the scene, all your rolls are treated as one step more exposed: `Safe` becomes `Risky`, `Risky` becomes `Desperate`, and `Desperate` rolls gain `+1` Trouble on a failure. You may recover by taking a slow action to draw breath and spending a Faith Point. | Things are going badly wrong, and you can’t cope. You need a Turn to pull yourself together.                             | Your abject behavior gets around town in a flash. Your Fame increases by one, but your Reputation moves three points in a negative direction.                                         |
...
If you end up with no dice at all you have no chance to succeed at this action—time to rethink your next step!
...
| Action difficulty | Modification |
| ----------------- | ------------ |
| Trivial           | +3           |
| Simple            | +2           |
| Easy              | +1           |
...
- Faith is spent to reduce Trouble before its outcome is rolled.
